...
#ifndef STREAM_APPLY_CHUNK_H
#define STREAM_APPLY_CHUNK_H
...
/**
* @brief A data cube that applies a pixel-wise user-defined function over chunks
*/

class stream_apply_chunk_cube : public cube {
   public:
    /**
        ...
        */
    static std::shared_ptr<stream_apply_chunk_cube>
    create(std::shared_ptr<cube> in, std::string cmd, uint16_t nbands,
        ...
        std::shared_ptr<stream_apply_chunk_cube> out = std::make_shared<stream_apply_chunk_cube>(in, cmd, nbands,
                                                                                                 names, keep_bands);
   ...
   public:
    stream_apply_chunk_cube(std::shared_ptr<cube> in, std::string cmd, uint16_t nbands,
   ...
   public:
    ~stream_apply_chunk_cube() {}

    std::shared_ptr<chunk_data> read_chunk(chunkid_t id) override;

    json11::Json make_constructible_json() override {
        json11::Json::object out;
        out["cube_type"] = "stream_apply_chunk_cube";
        out["cmd"] = _cmd;
        out["nbands"] = _nbands;
        out["names"] = _names;
