...
std::shared_ptr<chunk_data> stream_apply_chunk_cube::read_chunk(chunkid_t id) {
    GCBS_TRACE("stream_apply_chunk_cube::read_chunk(" + std::to_string(id) + ")");
    ...
    if (!f_in_stream.is_open()) {
        ...
        throw std::string(
            "ERROR in stream_apply_chunk_cube::stream_chunk_file(): cannot write streaming input data to file '" + f_in +
            "'");
    }
